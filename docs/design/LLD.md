...
- **Assistant/RAG:** use an allowlist of CUET sources, source metadata, chunk retrieval, a no-context result, and citations. Vector engine and embedding model remain unselected. Retrieved text is untrusted data, never an instruction or source of API permissions. See [RAG sequence](Sequence_Diagram.md#campus-assistant-query).
