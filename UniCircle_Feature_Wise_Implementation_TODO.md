...
The approved ERD is not in this repository, so Phase 6 establishes infrastructure only. The reviewed baseline migration creates only Alembic's version table; feature tables and their data seeds remain in Phase 7.
...
- [x] Add an opt-in timestamp audit mixin; defer feature IDs/relationships until the approved ERD is available.
