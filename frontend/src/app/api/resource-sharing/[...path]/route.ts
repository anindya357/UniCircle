...
const allowed: Record<string, RegExp[]> = {
  GET: [
    ...
    /^conversations$/,
    new RegExp(`^conversations/${uuid}/messages$`),
  ],
  POST: [
    /^resource-requests$/,
    new RegExp(`^resource-requests/${uuid}/decision$`),
    new RegExp(`^conversations/${uuid}/messages$`),
  ],
  PATCH: [/^resource-profile\/me$/],
};
...
async function proxy(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: { message: "Resource sharing is temporarily unavailable." } },
      { status: 503 },
    );
  }
}
